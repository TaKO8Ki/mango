...
pub fn text(data: String) -> node::Node {
  node::Node {
    children: Vec::new(),
    node_type: node::NodeType::Text(data),
  }
}

pub fn elem(name: String, attrs: node::AttrMap, children: Vec<node::Node>) -> node::Node {
  node::Node {
    children: children,
    node_type: node::NodeType::Element(node::ElementData {
      tag_name: name,
      attributes: attrs,
    }),
  }
...
pub fn parse(source: String) -> node::Node {
  ...
  if nodes.len() == 1 {
    ...
    elem("html".to_string(), std::collections::HashMap::new(), nodes)
  }
}
